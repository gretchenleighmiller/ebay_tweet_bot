...
class EbayParser():
    ...
    def __init__(self, config):
        # using a list of tuples instead of a dict because
        # eBay API is VERY sensitive about parameter order
        self.payload = [
    ...
    def make_payload(self):
        ...
        # are differentiated by a 0-indexed counter
        ...
        for i, item_filter in enumerate(filters, start=1):
            self.payload.append(('itemFilter({!s}).name'.format(i),
                                 item_filter['name']))
            self.payload.append(('itemFilter({!s}).value'.format(i),
                                 item_filter['value']))
